...
export const ScheduleMatrix: React.FC<ScheduleMatrixProps> = ({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white p-4 rounded-md border border-[#E0E0E0] shadow-2xs no-print">
        <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-gray-100">
          <div className="flex items-center space-x-2">
            <Filter className="w-4 h-4 text-[#00324D]" />
          ...
          <div className="flex items-center space-x-2">
            <button
              onClick={handlePrint}
              className="p-1.5 sm:px-3 sm:py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-semibold rounded flex items-center space-x-1 transition-colors cursor-pointer"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3 mt-3">
          ...
          <div>
            ...
            <select
              ...
              className="w-full p-1.5 bg-white border border-gray-300 rounded text-xs focus:ring-1 focus:ring-[#39A900] focus:outline-hidden"
            >
        ...
        <table className="w-full border-collapse min-w-[900px]">
          ...
          <thead>
            <tr className="bg-[#00324D] text-white">
              <th className="p-3 text-left text-xs font-bold uppercase tracking-wider w-28 border-r border-[#002236]">
              ...
              {DIAS_SEMANA.map(dia => (
                <th
                  key={dia.id}
                  className="p-3 text-center text-xs font-bold uppercase tracking-wider border-r border-[#002236] last:border-r-0"
                >
                  <div>{dia.nombre}</div>
                  <div className="text-[10px] text-gray-300 font-normal">Jornada Activa</div>
                </th>
              ))}
            </tr>
          </thead>
          ...
          <tbody className="divide-y divide-gray-200">
            {TIME_SLOTS.map((slot, sIdx) => (
              <tr key={sIdx} className="hover:bg-gray-50/50 transition-colors">
                {/* Time Slot Header */}
                <td className="p-3 text-xs font-bold text-gray-700 bg-[#F5F5F5] border-r border-gray-200 align-top whitespace-nowrap">
                  <div className="text-[#00324D]">{slot.label}</div>
                  <div className="text-[10px] text-gray-400 font-normal mt-0.5">2 Horas</div>
                </td>
                ...
                {DIAS_SEMANA.map(dia => {
                  ...
                  return (
                    <td
                      key={dia.id}
                      className="p-1.5 border-r border-gray-200 last:border-r-0 align-top schedule-grid-cell"
                    >
                      {cellHorarios.length === 0 ? (
                        /* Empty State: Dotted border, click to add */
                        isAdmin ? (
                          <button
                            onClick={() => onOpenCreateModal(dia.id, slot.start)}
                            className="w-full h-full min-h-[64px] border border-dashed border-gray-200 hover:border-[#39A900] hover:bg-[#f5fcea]/40 rounded p-2 text-center text-gray-400 hover:text-[#226d00] transition-all flex flex-col items-center justify-center group cursor-pointer"
                          ...
                          <div className="w-full min-h-[64px] rounded border border-gray-100 bg-[#FAFAFA] flex items-center justify-center text-[10px] text-gray-400">
                            Libre
                          </div>
                        )
                      ) : (
                        ...
                        <div className="space-y-1.5">
                          {cellHorarios.map(h => {
                            ...
                            return (
                              <div
                                key={h.id}
                                className="bg-white border border-gray-200 rounded p-2 shadow-2xs border-l-4 border-l-[#39A900] hover:shadow-sm transition-all text-left relative group"
                              >
                                {/* Header: Time & Program */}
                                <div className="flex items-center justify-between text-[11px] font-bold text-[#00324D]">
                                  <span className="bg-[#e9f1df] text-[#226d00] px-1 rounded text-[10px] font-bold">
                                ...
                                {/* Subject / Competence */}
                                <div className="text-xs font-semibold text-gray-800 mt-1 line-clamp-2 leading-tight">
                                  {h.materia_competencia}
                                </div>

                                {/* Details: Instructor & Room */}
                                <div className="mt-1.5 pt-1 border-t border-gray-100 flex flex-col space-y-0.5 text-[11px] text-gray-600">
                                  ...
                                  <div className="flex items-center space-x-1 truncate" title={ambiente?.nombre_ambiente}>
                                    <MapPin className="w-3 h-3 text-[#0288D1] shrink-0" />
                                ...
                                {isAdmin && (
                                  <div className="absolute top-1.5 right-1.5 opacity-0 group-hover:opacity-100 transition-opacity bg-white/95 rounded shadow-xs p-0.5 flex space-x-1 border border-gray-200 no-print">
                                    <button
                                      onClick={() => onEditHorario(h)}
                                      className="p-1 text-gray-500 hover:text-[#00324D] hover:bg-gray-100 rounded cursor-pointer"
                                    ...
                                    <button
                                      ...
                                      className="p-1 text-gray-500 hover:text-[#D32F2F] hover:bg-red-50 rounded cursor-pointer"
